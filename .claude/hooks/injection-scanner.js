#!/usr/bin/env node
// injection-scanner — PostToolUse Read hook.
// Scans file content returned by the Read tool for prompt-injection patterns.
// Catches poisoned content at ingestion, before it settles into conversation
// context. Long sessions get compressed, and the summariser cannot distinguish
// user instructions from content read out of external files — poisoned
// instructions that survive compression become indistinguishable from trusted
// context. This hook warns at ingestion time.
//
// Action: advisory warning (does not block).
// Severity: LOW (1-2 patterns), HIGH (3+ patterns).
//
// False-positive exclusion: security docs and hook source files legitimately
// contain injection-like strings.

const path = require('path');

// Patterns targeting instructions designed to survive context compression.

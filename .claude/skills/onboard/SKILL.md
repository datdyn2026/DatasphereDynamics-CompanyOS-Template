...
Immediately after the user answers -- BEFORE acting on any selection -- record every choice in `.claude/.extras`, one line per item, ALL initially marked `pending` (`session-changelog: pending`, `claude-mem: pending`, ...), or the single line `none: skipped` if nothing was picked. Only flip a line to `installed` after that extra's copy actually succeeded -- never mark ahead of the work. This file is how an interrupted run knows this step was already answered.
...
3. If `.claude/.extras` contains `pending` items, print one consolidated "Your extras checklist" with the exact commands -- one list, so the user restarts once, not per-extra. Include ONLY the items still marked `pending` -- never show commands for add-ons the user didn't pick. Commands per item:

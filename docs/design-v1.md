...
│   └── connections.md         # registry of systems/tools the client uses (CRM, email, etc.)
├── decisions/
│   └── log.md                 # append-only decision log (date, decision, why)
├── inbox/                     # append-only capture: drop anything, triage later
...
- Skill runs a ~7-question interview (15 min): business, model, team, top 3 goals, voice samples (paste 2-3 things you wrote), tools used, biggest time sink.
- Generates: all `company/*.md`, `GOALS.md`, personalizes `CLAUDE.md` header, seeds `workflows/` with 1 SOP from the "biggest time sink" answer, writes `.claude/.onboarded`.
- Ends by teaching the 3 daily commands: `/morning`, drop-to-`inbox/`, `/weekly-review`.

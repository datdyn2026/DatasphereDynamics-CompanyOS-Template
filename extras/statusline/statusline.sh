...
# Hardened for stdin-only mode: empty by default so refresh_usage_api() returns at
# its `[ ! -f "$CREDENTIALS_FILE" ]` guard before any token read or network call.
# (5h/context come from native stdin; SHOW_WEEKLY=0 so the API is never needed.)
...
if [ -n "$CWD" ]; then
    ...
    # strip control bytes — path comes from untrusted JSON (terminal OSC injection)
    DIR_DISPLAY="${DIR_DISPLAY//[$'\x01'-$'\x1f'$'\x7f']/}"
    # left-truncate very long paths, keeping the tail (mirrors BRANCH cap above)
    [ "${#DIR_DISPLAY}" -gt 30 ] && DIR_DISPLAY="...${DIR_DISPLAY: -27}"
fi
